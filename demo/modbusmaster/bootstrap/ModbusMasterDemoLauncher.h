...
inline int run_modbusmaster_demo()
{
    ...
    // ============================================================
    {
        Logger::info("[ModbusMasterDemo] === Round 1: create -> run -> destroy ===");
        ...
        auto host = ApplicationBootstrap::create(
            ...
            [](EventBus& bus) -> std::unique_ptr<TimerManager> {
                ...
                timers->add_periodic_timer("modbus_poll", 1000, [&bus]() {
                    ...
                    req.device_name = "demo-device";
                    req.reg_type = ModbusRegisterType::HOLDING_REGISTER;
                    req.slave_id = 1;
                    req.addr = 100;
                    req.count = 2;
        ...
        if (!host)
        {
            Logger::error("[ModbusMasterDemo] Round 1: bootstrap failed");
            return 1;
        }
        host->init();

        Logger::info("[ModbusMasterDemo] Round 1: running for 5 seconds");
        ...
        host->destroy();
        Logger::info("[ModbusMasterDemo] Round 1: destroyed");
    }

    // ============================================================
    // 第二轮：重新创建 → 运行 → 销毁（验证 destroy() 后可再次创建）
    // ============================================================
    {
        Logger::info("[ModbusMasterDemo] === Round 2: create again after destroy ===");
        ...
        if (!host)
        {
            Logger::error("[ModbusMasterDemo] Round 2: bootstrap failed");
            return 1;
        }
        host->init();

        Logger::info("[ModbusMasterDemo] Round 2: running for 5 seconds");
        ...
        host->destroy();
        Logger::info("[ModbusMasterDemo] Round 2: destroyed");
    }

    Logger::info("[ModbusMasterDemo] Both rounds completed successfully");
